"""Settings the matcher owns, layered onto the shared floor.

Every field here is read by `themis_matcher` and by no other member -- that is the
test for whether a knob belongs in this file rather than in
`themis_shared.config`. The two genuinely shared fields, `database_url` and
`matcher_base_url`, are inherited and keep their unprefixed environment names.

Everything declared below is `MATCHER_`-prefixed, so `embedding_model` is
`MATCHER_EMBEDDING_MODEL`. The prefix is what stops the matcher's LLM settings
colliding with the scraper's: both packages have an `llm_model`, they mean
different models (a query parser here, an extraction fallback there), and before
the prefixes existed the scraper had to import the two settings objects side by
side under different names to keep them apart.
"""
...
class MatcherSettings(Settings):
    """Config for the matching engine: parsing, embedding, retrieval, synthesis."""
    ...
    # 8100 rather than 8000 on purpose -- both this and the gateway's MCP server
    # run side by side on a laptop, and sharing that default would make them
    # collide.
