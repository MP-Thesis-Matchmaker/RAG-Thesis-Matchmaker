...
Until 2026-08-24 `themis_zora` kept a parallel set (its own `ZoraPublication`, its own
...
that nothing pinned together. `themis_zora` was also the only package importing nothing
from here. One model per shape is what makes that class of bug impossible rather
than merely documented; `themis_zora.mapping` now holds the mapping and imports the
models.
...
| `ZoraPublication` | `sources.py` | One ZORA publication, and exactly what the harvester writes as a `publication` row. 15 fields: `id`, `title`, `abstract`, `authors`, `uzh_authors`, `author_authority_map`, `year`, `keywords`, `department`, `owning_collection_uuid`, `language`, `publication_type`, `doi`, `url`, `accessioned`. |
...
were required `str`, so `themis_matcher.indexing.sources` satisfied them with `or ""` when
...
- `ZoraPublication` / `ZoraPerson` / `ZoraOrgUnit` — in `themis_zora.mapping`, on the
  way from the harvester's normalized dicts into Postgres. A malformed record fails
  the run rather than reaching a table.
- `ZoraPublication` / `ThesisPosting` — in `themis_matcher.indexing.sources`, both when
...
**Implemented and tested.** `libs/shared/tests/test_contracts.py` covers the source models
including `AuthorAuthority`'s `cris`/`orcid` discrimination, the two entity
mirrors, and the nullability above; `projects/zora/tests/test_mapping.py` covers the
...
- **`uzh_authors` is wider than its name.** It holds authors carrying *any* DSpace
  ...
  [`../zora/README.md`](../../../../../projects/zora/README.md); fixing it is the next task.
- **`ThesisPosting.language` has no producer.** The column, this field and the
  reader all exist; `themis_scraper.normalize.to_posting` never sets it, so it is
  always `None`.
...
- `ZoraPerson` and `ZoraOrgUnit` have **no consumer outside `themis_zora`** yet.
  `themis_matcher.indexing.SourceReader` exposes only `publications()` and `postings()`, so
